...
const ManageMembers = () => {
  ...
  const fetchMembers = async () => {
    ...
    try {
      const data = await getAllMembers(searchTerm.trim(), pageNumber, pageSize);
  ...
  const handleInputChange = (e) => {
    ...
    setFormData((previous) => ({
      ...previous,
      [name]: name === "gender" || name === "status" ? Number(value) : value,
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(serverMessage || "Operation failed. Please try again.");
    }
  };
    ...
    <>
      ...
      <div className="mb-6 flex flex-wrap gap-4">
        <div className="w-full rounded-xl border border-gray-100 bg-white p-4 shadow-sm sm:w-48">
          ...
          <p className="mt-2 text-2xl font-bold text-blue-600">{totalCount}</p>
        </div>
      </div>
